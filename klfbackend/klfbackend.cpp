...
KLFBackend::klfOutput KLFBackend::getPdfPage(const klfInput& in, const klfSettings& settings)
{
    ...
    QMutexLocker mutexlocker(&__mutex);
      ...
      { // execute latex
        ...
        args << settings.pdflatex << dir_native_separators(fnTex);

        qDebug("%s: %s:  about to exec pdflatex...", KLF_FUNC_NAME, KLF_SHORT_TIME) ;
        bool r = proc.startProcess(args, execenv);
        qDebug("%s: %s:  pdflatex returned.", KLF_FUNC_NAME, KLF_SHORT_TIME) ;

        if (!r) {
          res.status = KLFERR_NOLATEXPROG;
          res.errorstr = QObject::tr("Unable to start PdfLatex program %1!", "KLFBackend")
        .arg(settings.latexexec);
          return res;
        }
        if (!proc.processNormalExit()) {
          res.status = KLFERR_LATEXNONORMALEXIT;
          res.errorstr = QObject::tr("PdfLatex was killed!", "KLFBackend");
          return res;
        }
        if (proc.processExitStatus() != 0) {
          res.status = KLFERR_PROGERR_LATEX;
          res.errorstr = progErrorMsg("PdfLaTeX", proc.processExitStatus(), proc.readStderrString(),
        ...
        if (!QFile::exists(fnPdf)) {
          ...
          res.errorstr = QObject::tr("Pdf file didn't appear after having called PdfLatex!", "KLFBackend");
          return res;
        }
      ...
      QFile pdffile(fnPdf);
      bool r = pdffile.open(dev_READONLY);
      if ( ! r ) {
