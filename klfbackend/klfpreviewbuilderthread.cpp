...
void KLFPreviewBuilderThread::run()
{
  ...
  for (;;) {
    ...
    if ( input.latex.trimmed().isEmpty() ) {
      ...
      // and GO!
        if(settings.outputType==QString("PdfPage")){
          output = KLFBackend::getPdfPage(input,settings);
        } else {
    ...
    if(settings.outputType==QString("PdfPage")){
        if(output.status!=0) qDebug()<<output.errorstr;
        emit previewPdfAvailable(output.pdfdata, output.status !=0);
    } else {
